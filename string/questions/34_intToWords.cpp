#include<bits/stdc++.h>
using namespace std;

class Solution
{
public:
    vector<pair<int, string>> v = {{1000000000, "Billion"}, {1000000, "Million"}, {1000, "Thousand"}, {100, "Hundred"}, {90, "Ninety"}, {80, "Eighty"}, {70, "Seventy"}, {60, "Sixty"}, {50, "Fifty"}, {40, "Forty"}, {30, "Thirty"}, {20, "Twenty"}, {19, "Nineteen"}, {18, "Eighteen"}, {17, "Seventeen"}, {16, "Sixteen"}, {15, "Fifteen"}, {14, "Fourteen"}, {13, "Thirteen"}, {12, "Twelve"}, {11, "Eleven"}, {10, "Ten"}, {9, "Nine"}, {8, "Eight"}, {7, "Seven"}, {6, "Six"}, {5, "Five"}, {4, "Four"}, {3, "Three"}, {2, "Two"}, {1, "One"}};

    string empty = "";
    string space = " ";

    string numberToWordsInner(long long int num)
    {
        for (const auto &x : v)
        {
            if (num >= x.first)
            {
                return (num >= 100 ? numberToWords(num / x.first) + space : empty) +
                       x.second + (num % x.first == 0 ? empty : space + numberToWords(num % x.first));
            }
        }
        return "";
    }
    string numberToWords(long long int num)
    {

        if (num == 0)
        {
            return "Zero";
        }

        for (const auto &x : v)
        {
            if (num >= x.first)
            {
                return (num >= 100 ? numberToWordsInner(num / x.first) + space : empty) +
                       x.second + (num % x.first == 0 ? empty : space + numberToWordsInner(num % x.first));
            }
        }

        return "";
    }
};

int main(){
    Solution s;
    cout<<s.numberToWords(8772829392);
}